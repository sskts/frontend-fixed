...
export default form(
    ...
    form.field('creditNumber', 'クレジットカード番号').trim().required('', '%sが未入力です')
        .regex(/^(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[0-9]{15})$/, '%sが不適切です'),
    form.field('creditMonth', '有効期限（月）').trim().required('', '%sが未入力です'),
    form.field('creditYear', '有効期限（年）').trim().required('', '%sが未入力です'),
    form.field('creditName', 'カード名義人').trim().required('', '%sが未入力です')
        .regex(/^[A-Z]+[\s|　]+[A-Z]+[\s|　]*[A-Z]+$/, '%sが不適切です'),
    form.field('creditCord', 'セキュリティーコード').trim().required('', '%sが未入力です')
